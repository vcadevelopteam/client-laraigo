	...
	return (
		<div>
			...
			<Search panTo={panTo}
				...
				cleanDataAddres={cleanDataAddres}
				marker={marker}
				token={token} 
			/>
		</div>
...
function Search({ panTo, setMarker, directionData, setDirectionData, cleanDataAddres, marker, token }) {
		...
		<div style={{ width: "100%" }}>
			...
			<div className="search">
				...
				<div className={classes.containersearch}>
					...
					{(token?.split("-").pop() === "t") && (
						<div className="row-zyx" style={{ marginBottom: "0px" }}>
							<TextField label="Nro. dpto, piso interior" multiline variant="outlined" size="small" className="col-6" maxRows={4} value={directionData.departmentnumber} onChange={(e) => { setDirectionData((prev) => ({ ...prev, departmentnumber: e.target.value })) }} />
							<TextField label={t(langKeys.extraInformation)} multiline variant="outlined" size="small" className="col-6" maxRows={4} value={directionData.reference} onChange={(e) => { setDirectionData((prev) => ({ ...prev, reference: e.target.value })) }} />
						</div>
					)}

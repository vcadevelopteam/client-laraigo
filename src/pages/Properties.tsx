...
const Properties: FC = () => {
    ...
    const fetchData = () => dispatch(getCollection(getDistinctPropertySel(categoryFilter, levelFilter)));
...
const DetailNivelProperty: React.FC<ModalProps> = ({ data: { row, edit }, index, fields, onChangeSelectValue, register, errors, setValue, domainTable, extraData, watch }) => {
                ...
                if (edit) {
                    valueInput =
                        <FieldEditArray
                            className={classes.mb2}
                            error={errors?.table?.[index]?.propertyvalue?.message}
                            inputProps={{ step: 1, min: 0, pattern: "[0-9]{0,4}", maxlength: "4" }}
                            label={t(langKeys.value)}
                            onChange={(value) => setValue(`table.${index}.propertyvalue`, value)}
                            type="text"
                            valueDefault={row ? (row.propertyvalue || "") : ""}

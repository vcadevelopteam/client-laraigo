...
import GenerateLabelDialog from "../dialogs/GenerateLabelDialog";
import GenerateReferralGuideDialog from "../dialogs/GenerateReferralGuideDialog";
...
const InventoryConsumptionDetail: React.FC<DetailProps> = ({
    data: { row, edit },
    setViewSelected,
    fetchData,
    fetchDataAux,
    ...
    const [openModalReferralGuide, setOpenModalReferralGuide] = useState(false);
        ...
        if (waitSave) {
            if (!executeRes.loading && !executeRes.error) {
                ...
                const errormessage = t(executeRes.code ?? "error_unexpected_error", {
                    module: t(langKeys.warehouse).toLocaleLowerCase(),
                });
                ...
                execute(
                    {
                        ...
                        detail: [
                            ...dataDetail
                                ...
                                .map((x) =>
                                    inventoryConsumptionDetailIns({
                                        ...data,
                                        ...x,
                                        p_tableid: x?.inventoryconsumptionid || 0,
                                    })
                                ),
                        ],
                    },
                                ...
                                if (row?.status === "COMPLETADO") {
                                    ...
                                    dispatch(
                                        showSnackbar({
                                            show: true,
                                            severity: "error",
                                            message: t(langKeys.error_generate_label),
                                        })
                                    );
                                }
                            }}
                            referralguide={() => {
                                setOpenModalReferralGuide(true)
                            }}

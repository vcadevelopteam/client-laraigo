...
const AdjustStandardCostDialog: React.FC<{
  ...
  useEffect(() => {
    if (!data.loading && !data.error) {
      const newdata: any = data.data.reduce(
        (acc: any, x) => [...acc, { ...x, oldstandarcost: x.standarcost }],
        []
      );
  ...
  const columns = React.useMemo(
    () => [
      ...
      },
      {
        ...
        accessor: "oldstandarcost",
        type: "number",
        sortType: "number",
        Cell: (props: any) => {
          const row = props.cell.row.original;
          return parseFloat(row.oldstandarcost);
        },
      },
      {
        Header: `${t(langKeys.new)} ${t(langKeys.standard_cost)}`,
  ...
  return (
    <DialogZyx
      open={openModal}
      title={t(langKeys.adjuststandardcost)}
      maxWidth="md"
    >

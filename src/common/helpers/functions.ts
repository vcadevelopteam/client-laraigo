...
export function extractVariables(text: string, array: string[] = []): string[] {
    ...
    return Array.from(
        new Set([
            ...array,
            ...Array.from(text.matchAll(rex), (m: any[]) => m[0].replace(/[{]{2}/, "").replace(/[}]{2}/, "")),
        ])
    );
}
...
export function richTextToString(data: Dictionary[]) {
    try {
        return data
            .reduce(
                (ac: Dictionary[], c: Dictionary) => [
                    ...ac,
                    ...c?.children?.map((m: Dictionary) => (m?.children ? `- ${m?.children[0]?.text}` : m?.text)),
                ],
                []
            )
            .join("\n");
    ...
    return new Promise((res, rej) => {
        import("xlsx").then((XLSX) => {
            ...
            reader.onload = (event: any) => {
                ...
                let rowsx = XLSX.utils.sheet_to_json(workbook.Sheets[wsname]).map((row: any) =>
...
export const uploadExcelBuffer = (buffer: any) => {
    return new Promise((res, rej) => {
        import("xlsx").then((XLSX) => {
            ...
            const rowsx = XLSX.utils.sheet_to_json(workbook.Sheets[sheetName]).map((row: any) =>
                Object.keys(row).reduce((obj: any, key: any) => {
                    obj[key.trim()] = row[key];
                    return obj;
                }, {})
    ...
    return new Promise((res, rej) => {
        import("xlsx").then((XLSX) => {
            ...
            reader.onload = (event: any) => {
                ...
                const containsRequiredOrOptional = headers.some(
                    (header: string) => header.includes("|Opcional|") || header.includes("|Obligatorio|")
                );
                ...
                if (duplicateHeaders.size > 0) {
                    ...
                    dispatch(
                        showSnackbar({
                            show: true,
                            severity: "warning",
                            message: `Se eliminaron las filas duplicadas encontradas para los headers: "${duplicatesMessage}"`,
                        })
                    );
                ...
                if (duplicatedRows.size > 0) {
                    dispatch(
                        showSnackbar({ show: true, severity: "warning", message: "Se eliminaron personas duplicadas" })
                    );
                }
...
export const dateToLocalDate = (date: string, returnType = "string"): string | Date => {
    ...
    else return dateCleaned;
};

export const todayDate = (): Date => {
    return new Date(
        new Date(new Date().getTime() - new Date().getTimezoneOffset() * 60 * 1000).toISOString().substring(0, 10) +
            "T00:00:00"
    );
};
...
export const convertLocalDate = (
    ...
    validateWithToday: boolean = false,
    subtractHours: boolean = true
): Date => {
...
export const getSecondsUntelNow = (date: Date, regressive: boolean = false): number =>
    !regressive
        ? Math.floor((new Date().getTime() - date.getTime()) / 1000)
        : Math.floor((date.getTime() - new Date().getTime()) / 1000);

export const getTimeBetweenDates = (date1: Date, date2: Date): string =>
    secondsToTime(Math.floor((date2.getTime() - date1.getTime()) / 1000));
...
export const cleanedRichResponse = (data: Dictionary[], variablesContext: Dictionary = {}): Dictionary[] => {
    return data
        .filter(
            (x: Dictionary) =>
                ["content", "structured message", "action"].includes(x.plugincategory) &&
                !["closestlocation"].includes(x.pluginid)
        )
        .map((y: Dictionary) => {
            ...
            if (y.config.randomlist) {
                ...
                } else content = y.config.randomlist[0].value;
            }
...
export const randomText = (length = 8, use_upper = false, use_number = false, use_especial = false) => {
    ...
    if (use_upper) chars = [...chars, ...upper.split("")];
    if (use_number) chars = [...chars, ...number.split("")];
    if (use_especial) chars = [...chars, ...especial.split("")];
    return Array(length)
        .fill(null)
        .reduce((r, e, i) => {
            let c = "";
            if (i > 0)
                c = chars.filter((c) => c !== r[i - 1])[
                    randomInterval(0, chars.filter((c) => c !== r[i - 1]).length - 1)
                ];
            else c = chars[randomInterval(0, chars.length - 1)];
            r.push(c);
            return r;
        }, [])
        .join("");
...
export const templateMaker = (data: any[], header: string[]) => {
    ...
    for (let i = 0; i < max; i++) {
        header.forEach((d, j) => {
            ...
            if (datakey === data[j][datakey]) temp[i][d] = Object.keys(data[j] || {})[i];
            else temp[i][d] = `${datakey} - ${data[j][datakey]}`;
        });
    }
    return temp;
};
...
export const calculateDateFromMonth = (year: number, month: number) => {
    ...
    const daysPreviewMonth = Array.from(Array(dayPreviewMonth).keys())
        .map((x) => {
...
export const stringBDTimestampToLocalDate12hr = (timestamp: string) => {
    ...
    const formattedDate = `${day < 10 ? "0" : ""}${day}-${month < 10 ? "0" : ""}${month}-${year} ${displayHours}:${
        minutes < 10 ? "0" : ""
...
export const dayNames2 = ["monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday"];
...
export const validateIsUrl = (text: string) => {
    ...
    const regx = /(\b(https?|ftp|file):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/gi;
    const matches = sanitizedHTML.match(regx);
    if (!matches || matches.length === 0) return sanitizedHTML.replace(/%(?=[A-Za-z0-9])/g, "% ");
    const replaces =
        matches?.reduce(
            (acc, item, index) => acc.replace(item, `<a href="###${index}###" target="_BLANK">###${index}###</a>`),
            text
        ) || text;
    return (
        matches?.reduce((acc, item, index) => acc.replace(new RegExp(`###${index}###`, "g"), item), replaces) || text
    );
};
...
export const loadScripts = (scripts: string[]) => {
    ...
    if (scripts.includes("recaptcha")) {
        ...
        scriptRecaptcha.src =
            "https://www.google.com/recaptcha/enterprise.js?render=6LeOA44nAAAAAMsIQ5QyEg-gx6_4CUP3lekPbT0n";
...
export function normalizeKeys(obj:any) {
    ...
    Object.keys(obj).forEach((key) => {
        const normalizedKey = removeAccents(key.trim().toLowerCase().replace(/\s+/g, "_"));
...
// Función para eliminar tildes y otros signos diacríticos
